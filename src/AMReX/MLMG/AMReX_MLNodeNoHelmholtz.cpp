...
#include "AMReX/MLMG/AMReX_MLNodeNoHelmholtz_K.H"
...
MLNodeNoHelmholtz::MLNodeNoHelmholtz (const Vector<Geometry>& a_geom,
...
MLNodeNoHelmholtz::~MLNodeNoHelmholtz ()
{}

void
MLNodeNoHelmholtz::define (const Vector<Geometry>& a_geom,
                         const Vector<BoxArray>& a_grids,
                         const Vector<DistributionMapping>& a_dmap,
                         const LPInfo& a_info,
                         const Vector<FabFactory<FArrayBox> const*>& a_factory)
{
    BL_PROFILE("MLNodeNoHelmholtz::define()");
...
void
MLNodeNoHelmholtz::unimposeNeumannBC (int amrlev, MultiFab& rhs) const
{
...
void
MLNodeNoHelmholtz::setSigma (int amrlev, const MultiFab& a_sigma)
{
    MultiFab::Copy(*m_sigma[amrlev][0][0], a_sigma, 0, 0, 1, 0);
}

void
MLNodeNoHelmholtz::compDivergence (const Vector<MultiFab*>& rhs, const Vector<MultiFab*>& vel)
{
    compRHS(rhs, vel, Vector<const MultiFab*>(), Vector<MultiFab*>());
}

void
MLNodeNoHelmholtz::compRHS (const Vector<MultiFab*>& rhs, const Vector<MultiFab*>& vel,
                          const Vector<const MultiFab*>& rhnd,
                          const Vector<MultiFab*>& a_rhcc)
{
    BL_PROFILE("MLNodeNoHelmholtz::compRHS()");
...
void
MLNodeNoHelmholtz::updateVelocity (const Vector<MultiFab*>& vel, const Vector<MultiFab const*>& sol) const
{
#if (AMREX_SPACEDIM == 2)
    bool is_rz = m_is_rz;
#endif
...
void
MLNodeNoHelmholtz::getFluxes (const Vector<MultiFab*> & a_flux, const Vector<MultiFab*>& a_sol) const
{
#if (AMREX_SPACEDIM == 2)
    bool is_rz = m_is_rz;
#endif
...
void
MLNodeNoHelmholtz::averageDownCoeffs ()
{
    BL_PROFILE("MLNodeNoHelmholtz::averageDownCoeffs()");
...
void
MLNodeNoHelmholtz::averageDownCoeffsToCoarseAmrLevel (int flev)
{
    const int mglev = 0;
...
void
MLNodeNoHelmholtz::averageDownCoeffsSameAmrLevel (int amrlev)
{
...
void
MLNodeNoHelmholtz::FillBoundaryCoeff (MultiFab& sigma, const Geometry& geom)
{
    BL_PROFILE("MLNodeNoHelmholtz::FillBoundaryCoeff()");
...
void
MLNodeNoHelmholtz::buildStencil ()
{
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(AMREX_SPACEDIM != 1,
                                     "MLNodeNoHelmholtz::buildStencil: 1d not supported");
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(!m_geom[0][0].IsRZ(),
                                     "MLNodeNoHelmholtz::buildStencil: cylindrical not supported for RAP");
...
void
MLNodeNoHelmholtz::fixUpResidualMask (int amrlev, iMultiFab& resmsk)
{
    if (!m_masks_built) buildMasks();
...
void
MLNodeNoHelmholtz::prepareForSolve ()
{
    BL_PROFILE("MLNodeNoHelmholtz::prepareForSolve()");
...
void
MLNodeNoHelmholtz::restriction (int amrlev, int cmglev, MultiFab& crse, MultiFab& fine) const
{
    BL_PROFILE("MLNodeNoHelmholtz::restriction()");
...
void
MLNodeNoHelmholtz::interpolation (int amrlev, int fmglev, MultiFab& fine, const MultiFab& crse) const
{
    BL_PROFILE("MLNodeNoHelmholtz::interpolation()");
...
void
MLNodeNoHelmholtz::averageDownSolutionRHS (int camrlev, MultiFab& crse_sol, MultiFab& crse_rhs,
...
void
MLNodeNoHelmholtz::restrictInteriorNodes (int camrlev, MultiFab& crhs, MultiFab& a_frhs) const
{
...
void
MLNodeNoHelmholtz::Fapply (int amrlev, int mglev, MultiFab& out, const MultiFab& in) const
{
    BL_PROFILE("MLNodeNoHelmholtz::Fapply()");
...
void
MLNodeNoHelmholtz::Fsmooth (int amrlev, int mglev, MultiFab& sol, const MultiFab& rhs) const
{
    BL_PROFILE("MLNodeNoHelmholtz::Fsmooth()");
...
void
MLNodeNoHelmholtz::normalize (int amrlev, int mglev, MultiFab& mf) const
{
    BL_PROFILE("MLNodeNoHelmholtz::normalize()");
...
void
MLNodeNoHelmholtz::compSyncResidualCoarse (MultiFab& sync_resid, const MultiFab& a_phi,
                                         const MultiFab& vold, const MultiFab* rhcc,
                                         const BoxArray& fine_grids, const IntVect& ref_ratio)
{
    BL_PROFILE("MLNodeNoHelmholtz::SyncResCrse()");
...
void
MLNodeNoHelmholtz::compSyncResidualFine (MultiFab& sync_resid, const MultiFab& phi, const MultiFab& vold,
                                       const MultiFab* rhcc)
{
    BL_PROFILE("MLNodeNoHelmholtz::SyncResFine()");
...
void
MLNodeNoHelmholtz::reflux (int crse_amrlev,
                         MultiFab& res, const MultiFab& crse_sol, const MultiFab& crse_rhs,
                         MultiFab& fine_res, MultiFab& fine_sol, const MultiFab& fine_rhs) const
{
    BL_PROFILE("MLNodeNoHelmholtz::reflux()");
...
#ifdef AMREX_USE_EB
void
MLNodeNoHelmholtz::buildIntegral ()
{
    if (m_integral_built) return;

    BL_PROFILE("MLNodeNoHelmholtz::buildIntegral()");
...
void
MLNodeNoHelmholtz::checkPoint (std::string const& file_name) const
{
...
#if (AMREX_SPACEDIM == 2)
void
MLNodeNoHelmholtz::fillIJMatrix (MFIter const& mfi, Array4<HypreNodeLap::Int const> const& nid,
