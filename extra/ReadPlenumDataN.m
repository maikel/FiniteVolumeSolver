function [X, Y, Z, time, data] = ReadPlenumDataN(directory_path, k0, nlimit, pattern)
  if nargin == 3
    pattern = '/*.dat';
  end
  ...
  ntimesteps = min(ntimesteps - k0 + 1, nlimit);
  ...
  for k = 1 :  ntimesteps
    filename = sprintf('%s/%s', directory_path, all_filenames{k + k0 - 1});
    fprintf('Process %s\n', filename);
