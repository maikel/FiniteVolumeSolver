...
if (len(sys.argv)>1):
   ...
   dataPath = FVS_path+"/build_3D-Release/MultiTube/Slices"
   inputFilePath = FVS_path+"/examples/AMReX/EB/3D/"
...
for tube_id in range(n_tubes):
  ...
  filename_basic = dataPath+'/Tube_{}.h5'.format(tube_id)
  ...
  rhoF_data = datas[slice_start:, datas_dict['Species_3'], :]
  p_data = datas[slice_start:, datas_dict['Pressure'], :]
  c_data = datas[slice_start:, datas_dict['SpeedOfSound'], :]
  rhoX_data = datas[slice_start:, datas_dict['Species_6'], :]
  T_data = rho_data = datas[slice_start:, datas_dict['Temperature'], :]
  ...
  titles = ['Temperature [K]', 'Pressure [bar]', 'Local Machnumber [-]', 'H2 Massfraction [-]', 'O2 Massfraction [-]']
  datas = [T_data * T_ref, p_data, Ma, X, F_data]
