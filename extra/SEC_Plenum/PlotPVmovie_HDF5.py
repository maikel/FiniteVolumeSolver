...
# filename_controlState = "{}/ControlState.h5".format(dataPath)
# print("Read in data from {}".format(filename_controlState))

# if RESTARTEDSIMULATION:
#   csData, csTimes, csDict = io.h5_load_restartedTimeseries(filename_controlState)
# else:
#   csData, csTimes, csDict = io.h5_load_timeseries(filename_controlState)
...
# we only take data from combustion tube
tube_id = 0
...
tube_velocity = datas[t_index_array, datas_dict['Momentum'], :] / tube_rho
...
if not Dimless:
  ...
  # TODO scale velocity right
  # tube_velocity *= pv_class.ref_velocity
else:
  specVol = np.power(rho, -1)
...
def stackTubeDataTo2D(Tube_datalist, nRepeats=2):
    ...
    for i, el in enumerate(Tube_datalist):
      el = np.squeeze(el)
      Tube_datalist[i] = np.stack(tuple(el for i in range(nRepeats)))
...
def plotFigure(i, step, valueDict, pv_class, PARALLEL=False):
  ...
  velocity = stackTubeDataTo2D([ tube_velocity[step,:] ], nRepeats=6)
  ...
  if PLOTVELOCITYARROWS:
    # velocity plot
    skip = 5
    props_dict = props('Velocity')
    velocity_x = velocity[0][:,::skip]
    velocity_y = np.zeros_like(velocity_x)

    x_quiver = np.linspace(*tube_extent_ax3[0:2], num=velocity_x.shape[1], endpoint=True)
