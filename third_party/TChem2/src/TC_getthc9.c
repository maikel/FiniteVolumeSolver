...
*/
int TCKMI_readValidLine(FILE *filein, char* linein)
{
  int len1 = 0;
...
int TCKMI_getthermo9(char *singleword, FILE *thermoin,
              element *listelem,int *Nelem,species *listspec,int *Nspec,
              int *ithermo,int *iread,int *ierror)
{
  ...
  if (*iread != 4) { (*ierror) = 300 ; return ( 1 ) ; }

  /* Set up the input file */
  thermofile = thermoin ;

  /* Check if file pointer is null or if file is empty */
  if ( thermofile == NULL  ) { (*ierror) = 320 ; return ( 1 ) ; }
  if ( feof(thermofile) > 0) { (*ierror) = 338 ; return ( 1 ) ; }
  ...
  while ( len1 == 0 )
  {
    ...
    if ( feof(thermofile) > 0) { (*ierror) = 340 ; return ( 1 ) ; }
  ...
  while (len1==0)
  {
    ...
    fgets(linein1,200,thermofile) ;
    if ( feof(thermofile) > 0) { (*ierror) = 342 ; return ( 1 ) ; }
    TCKMI_cleancharstring(linein1,&len1) ;
  }
  ...
  while ( feof(thermofile) == 0 )
  {
    ...
    if (strncmp(singleword,"END",3) == 0) { return ( 0 ) ; }

    TCKMI_readValidLine(thermofile,linein2) ;
    if ( feof(thermofile) > 0) { (*ierror) = 360 ; return (1) ;}
    ...
    if (len1==0) { (*ierror) = 370 ; return 1 ; }

    /* Get number of temperature ranges */
    len1=2 ;
    strncpy(singleword,linein2,len1) ; singleword[len1] = 0;
   ...
   /* Check if species is of interest */
	...
	strncpy(singleword,&linein2[itemp],2); singleword[2] = 0;
	...
	if (ipos1 == (*Nelem)) { (*ierror) = 380 ; return (1) ; }

	listspec[ipos].elemindx[i] = ipos1;

	/* Good element, find numbers */
	len1=6 ;
	strncpy(singleword,&linein2[itemp+2],len1); singleword[len1] = 0;
	TCKMI_checkstrnum(singleword,&len1,ierror) ;
	if (*ierror > 0) return 1;
	...
	if (strncmp(listelem[ipos1].name,"E",1) == 0)
      ...
      /* Update mass */
      len1=13;
      strncpy(singleword,&linein2[52],len1); singleword[len1] = 0;
      TCKMI_checkstrnum(singleword,&len1,ierror) ;
      if (*ierror > 0) return (1);
        ...
        for ( irng = 0; irng<listspec[ipos].Nth9rng; irng++ )
	{
	  ...
	  len1=11 ;
	  strncpy(singleword,&linein1[0],len1); singleword[len1] = 0;
	  TCKMI_checkstrnum(singleword,&len1,ierror) ;
	  if (*ierror > 0) return (1);
	  listspec[ipos].Nth9Temp[2*irng] = atof(singleword) ;

	  strncpy(singleword,&linein1[len1],len1); singleword[len1] = 0;
	  TCKMI_checkstrnum(singleword,&len1,ierror) ;
	  if (*ierror > 0) return (1);
	  ...
	  len1=1;
	  strncpy(singleword,&linein1[22],len1); singleword[len1] = 0;
	  TCKMI_checkstrnum(singleword,&len1,ierror) ;
	  if (*ierror > 0) return (1);
          ...
          for ( ip = 0; ip < MIN(Nth9npow,5); ip++ )
	  {
	    len1=16;
	    itemp = 0+len1*i;
	    strncpy(singleword,&linein2[itemp],len1) ; singleword[len1] = 0;
	    TCKMI_checkstrnum(singleword,&len1,ierror) ;
	    listspec[ipos].Nth9coefs[irng*9+pows[ip]+2] = atof(singleword) ;
	  }
          /* a6-a7 */
          for ( ip = 6; ip < MIN(Nth9npow,7); ip++ )
	  {
	    len1=16;
	    itemp = 0+len1*i;
	    strncpy(singleword,&linein3[itemp],len1) ; singleword[len1] = 0;
	    TCKMI_checkstrnum(singleword,&len1,ierror) ;
	    listspec[ipos].Nth9coefs[irng*9+pows[ip]+2] = atof(singleword) ;
	  }
          /* b1-b2 */
