...
int main(int argc, char** argv) {
  ...
  amrex::Geometry geom(domain, &rb, amrex::CoordSys::cartesian,
                       is_periodic.data());

  amrex::BoxArray box_array(domain);
  box_array.maxSize(max_grid_size);
  amrex::BoxArray node_boxes =
      amrex::convert(box_array, {AMREX_D_DECL(1, 1, 1)});
...
//       constexpr double fac = tpi * tpi * AMREX_SPACEDIM;
  ...
  // create cell-centered multifab to hold value of -sigma*grad(phi) at
  // cell-centers
  ...
  amrex::MultiFab::Copy(plotfile_mf, vel, 0, AMREX_SPACEDIM + 1, AMREX_SPACEDIM,
                        0);
